            ...
            setInterval(function(){
                for (var p in pzhs){
                    if (pzhs.hasOwnProperty(p)){
                        pzhs[p].sendMessageToAllPzp("keepAlive", "{}");
                    }
                }
            }, 30000);
        }catch(err){
    ...
    function getAllPzhList(userId, userObj) {
        ...
        for (myKey in config.trustedList.pzh) {
            if (config.trustedList.pzh.hasOwnProperty(myKey) && myKey !== userId
                ...
                if (pzhList){
                    list.push({url:myKey,
                        username:pzhList && pzhList.friendlyName,
                        email:pzhList && pzhList.email,
                        nickname: pzhList && pzhList.nickname });
                }
            }
        }
        return list;
